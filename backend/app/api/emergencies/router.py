...
def acknowledge_emergency(
    ...
    if emergency.status == EmergencyStatus.OPEN:
        ...
        log_action(db, AuditAction.EMERGENCY_ACKNOWLEDGED, actor_id=current_user.id, actor_email=current_user.email,
                   entity_type="Emergency", entity_id=emergency.id)
...
def resolve_emergency(
    ...
    db.commit()
    db.refresh(emergency)
    log_action(db, AuditAction.EMERGENCY_RESOLVED, actor_id=current_user.id, actor_email=current_user.email,
               entity_type="Emergency", entity_id=emergency.id)
...
def mark_false_alarm(
    ...
    db.commit()
    db.refresh(emergency)
    log_action(db, AuditAction.EMERGENCY_FALSE_ALARM, actor_id=current_user.id, actor_email=current_user.email,
               entity_type="Emergency", entity_id=emergency.id)
