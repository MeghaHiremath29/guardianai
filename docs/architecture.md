...
## Audit trail vs. emergency timeline (Phase 5)

There are two separate logging mechanisms in this codebase, deliberately
not merged:

- **`EmergencyTimeline`** narrates the story of a single incident (fall
  detected → emergency created → caretaker notified → acknowledged →
  resolved). It's scoped to one `Emergency` and shown on that emergency's
  detail view.
- **`AuditLog`** is system-wide and security-focused: every login attempt
  (successful or failed), every registration, every emergency status
  change, every video upload. It answers "who did what, when" across the
  whole system, not just within one incident, and is only visible to
  ADMIN users via `/audit-logs`.

Both are written through single, centralized functions —
`EmergencyTimeline` rows are added directly by `emergency_engine.py` and
the emergencies router, and every `AuditLog` write goes through
`app/services/audit.py::log_action()` — so the "never log secrets" rule
only has to be enforced in one place rather than at every call site.

## Analytics computation (Phase 5)

`/analytics/summary` and `/analytics/trends` compute every number directly
from the database at request time — there is no caching layer, materialized
view, or precomputed rollup table. This is the right tradeoff at this
project's scale (a handful of emergencies, not millions), and it guarantees
the numbers shown are never stale. If this needed to scale to a large
dataset, the natural next step would be a scheduled job that maintains
daily rollup rows, with the live endpoints falling back to computing only
the last day queried against the raw table.
